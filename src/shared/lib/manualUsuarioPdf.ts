...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      '4 tarjetas de estado (arriba): "Vehículos / Maquinaria ACTIVA" (operativos), "En MANTENIMIENTO" (equipos con al menos una solicitud de servicio registrada — al tocarla lleva al submódulo Servicio de Mantenimiento para darles seguimiento), "Próximos a Mantenimiento" (equipos cerca de su próximo servicio según el kilometraje/horómetro, dentro del margen de aviso) y "En ESTADO CRÍTICO" (mantenimiento vencido o fuera de servicio). Tocar Activa, Próximos a Mantenimiento o Crítico filtra la tabla a esos equipos.',
      ...
      'HRS acumuladas y alerta: la tabla muestra las HRS acumuladas (suma de las horas trabajadas, del horómetro vigente) y cuántas faltan para el próximo mantenimiento. El equipo se marca con ⚠️ solo cuando está cerca de cumplir su servicio (dentro del 10% del intervalo; ej.: cada 250 h avisa con ≤ 25 h), no desde la primera hora. Las horas acumuladas se traen de Combustible si el equipo está vinculado. El gráfico «Equipos por status» del Resumen es clickeable. «Desactivar» deja el equipo inactivo (se ve con «Ver inactivos» y se reactiva con «Activar»); además, el botón 🗑 Eliminar lo borra de forma definitiva (incluida su bitácora de mantenimientos), con confirmación previa.',
      'Bitácora / horómetro (🔧): registro cronológico (fecha, horómetro, kilometraje, aceite, refrigerante, gasoil, trabajo, mecánico, ubicación). Las HRS. trabajadas (lectura − lectura anterior) y el consumo Lts/h (gasoil ÷ HRS) se calculan solos, igual que el Excel. Avisa cuando el período supera la frecuencia de mantenimiento. También se puede indicar una "alerta de kilometraje": al acercarse (dentro del 10%) o alcanzar ese km, el equipo se avisa con una tarjeta en Servicio de Mantenimiento.',

...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Servicio Directo (pestaña): igual que la Compra Directa pero como SERVICIO (mano de obra, mantenimientos, reparaciones, recargas). No pasa por aprobación y NO entra al inventario; solo registra el gasto en Tesorería. Se crea con el mismo formato que la Solicitud de Servicio: uno o varios renglones con Categoría del servicio (se busca o se escribe), Tipo de servicio (caucho, aceite, pintura, repuesto… del catálogo, o uno nuevo), Equipo de Control de Maquinaria y Cantidad. Correlativo SD-AAAA-####, proveedor opcional (directorio o alta en el momento); las categorías y tipos nuevos se guardan en el catálogo de servicios. Al finalizar ("Cargar factura y monto") se adjunta la factura, se carga el monto por servicio, la categoría/subcategoría de gasto y la caja (con su billetera; si es Multimoneda, el pago repartido por moneda con tasa BCV) — el monto sale de la billetera real (egreso en Tesorería). Si el renglón tiene equipo, el servicio aparece en el historial del equipo (Maquinaria → Bitácora / Resumen → clic en el equipo). En proceso se puede eliminar; finalizado no.',

...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Indicar método de pago: cuando la OC está "Confirmada (indicar método de pago)" se eligen el/los método(s) de pago y el tipo de soporte (Nota de entrega / Factura). En ese paso también se puede CAMBIAR EL PROVEEDOR de la OC: como cambia el proveedor adjudicado, la OC VUELVE A APROBACIÓN DEL GERENTE GENERAL (no se envía a pagar en ese momento); los ítems y el total se mantienen. Tras la nueva aprobación se vuelve a indicar el método de pago.',
      'Pago en Tesorería (Órdenes pendientes por pagar): al abrir una OC para pagar se ve un resumen de la orden (solicitante, unidad solicitante, finalidad, motivo y notas), la condición de pago y la conversión $ ⇄ Bs. El pago se puede anclar a un gasto eligiendo categoría → subcategoría (listas buscables/filtrables), para que quede etiquetado como gasto además de pago de OC.',
      'Pagar varias OC del mismo proveedor: en el listado de órdenes por pagar se marcan varias con casillas (✓) y, si son del mismo proveedor, se pagan juntas desde una sola caja (un egreso por OC). Funciona incluso con las que están "Esperando método de pago" (no hace falta esperar a que Compras indique el método).',
  ...
  },
  {
    ...
    puntos: [
      ...
      'Observación por material: cada renglón del carrito admite una observación (ej.: "será trasladado para reparación") que aparece en el detalle y en la Orden de Salida.',

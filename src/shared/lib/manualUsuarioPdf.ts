...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Humedad Provisional y Humedad Final: debajo del laboratorio hay dos tablas, una al lado de la otra, con sus botones "+ Humedad Provisional" y "+ Humedad Final" para agregar filas. Provisional: se cargan Peso (Gr) Húmedos y Peso (Gr) seco; el sistema calcula % Humedad = (Húmedos - seco) / Húmedos x 100 y Merma peso H2O = Húmedos - seco; al pie, Promedio del lote del % (promedio simple) y Merma total (sumatoria). Final: se carga Peso (Kg) recogido; el % Humedad final aplica el Promedio del lote de la Provisional y Merma peso H2O = Peso recogido x % Humedad / 100; al pie, Peso recogido y Merma totales (sumatorias). Solo la humedad se muestra en %. Cada fila se puede eliminar y se guarda en tiempo real.',

...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Compra/Servicio Directo - flujo analista monta -> Tesorería paga: tres estados En proceso -> Por pagar -> Finalizada. El ANALISTA pulsa "Cargar factura y montos/monto", adjunta la factura y carga los montos: la compra/servicio pasa a "Por pagar" (etiqueta DIRECTO) y NO toca caja ni inventario. El pago lo hace SOLO Tesorería: en Tesorería -> "Órdenes pendientes por pagar" aparece el panel "Directos por pagar" (compras y servicios) y el contador del botón incluye los directos; el usuario de Tesorería pulsa "Pagar", elige la caja correspondiente (billetera / Multimoneda con tasa BCV) y la categoría/subcategoría de gasto -> el dinero sale de esa caja y queda Finalizada. En compras el stock NO entra al pagar: queda pendiente de recepción para el almacenista (ver abajo). El botón Pagar ya NO aparece en Compra/Servicio Directo.',
      'Compra Directa - recepción en inventario (almacenista): al pagarse una compra directa, el stock NO entra automáticamente. La compra aparece en Inventario -> Recepciones, panel "Compras directas por recibir" (el contador del botón las incluye). El almacenista pulsa "Recibir", ve el detalle (materiales, cantidades y costo) y elige el almacén / sub-almacén destino; al confirmar, cada material entra al inventario como ENTRADA (costo = gasto / cantidad -> PMP) en el almacén elegido. El detalle de la compra indica si está "pendiente de recepción" o ya "recibida" (en qué almacén y por quién). Si la compra está marcada "no ingresa al inventario" (materiales ya cargados a mano), no aparece para recibir.',
  ...
  },
  {
    ...
    puntos: [
      ...
      'Añadir valores / Guardar datos (independiente): la tabla de laboratorio es independiente de la de recepciones (kg). El botón "+ Añadir valores" agrega una fila de análisis químico; el N° Análisis es correlativo y se incrementa solo. Los N° cargados se gestionan en la barra "Análisis cargados" (editar el número y eliminar). Además del guardado al salir de cada celda, "Guardar datos" guarda de una vez todos los datos cargados sin borrar lo recién ingresado. Esos datos químicos se muestran después al Cerrar la recepción (en el detalle del cierre, como "Resultados de laboratorio").',

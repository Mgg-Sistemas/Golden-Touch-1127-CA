...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Compra/Servicio Directo - detalle, reabrir, editar y varias facturas: al hacer clic en la tarjeta (kanban) o en la fila (lista) se abre el detalle (cabecera + tabla de renglones + facturas). En "En proceso" el botón "Editar" (✏) permite cambiar materiales/servicios, cantidades, almacén/equipo y proveedor. En "Finalizada" el botón "Reabrir" (↺) devuelve el dinero a la caja (revierte el egreso en Tesorería, exacto incluso en Multimoneda, sin alterar la tasa promedio) y —en compras— revierte la entrada al inventario, dejándolo "En proceso" para corregirlo y volver a finalizarlo (si ese stock ya se consumió, el inventario queda en 0, no negativo). En el detalle también se pueden cargar VARIAS facturas o comprobantes (PDF o imagen), previsualizarlas y borrarlas individualmente.',
      ...
      'Compra/Servicio Directo - flujo analista monta -> Tesorería paga: tres estados En proceso -> Por pagar -> Finalizada. El ANALISTA pulsa "Cargar factura y montos/monto", adjunta la factura y carga los montos: la compra/servicio pasa a "Por pagar" (etiqueta DIRECTO) y NO toca caja ni inventario. El pago lo hace SOLO Tesorería: en Tesorería -> "Órdenes pendientes por pagar" aparece el panel "Directos por pagar" (compras y servicios) y el contador del botón incluye los directos; el usuario de Tesorería pulsa "Pagar", elige la caja correspondiente (billetera / Multimoneda con tasa BCV) y la categoría/subcategoría de gasto -> el dinero sale de esa caja, en compras entra al inventario (PMP) y queda Finalizada. El botón Pagar ya NO aparece en Compra/Servicio Directo.',
  ...
  },
  {
    ...
    puntos: [
      ...
      'Conciliación (vs Centros de Acopio): compara el Peso KG total recibido contra lo reportado por los centros de acopio (incluye aliados). Con "Nueva conciliación" se crea una: la primera vez se pide el N° de recepción y luego se sugiere incremental (editable). Centros de Acopio: lista editable de centro/aliado + Saldo KG (añadir/editar/eliminar); el Peso KG TOTAL es editable. Totales: Kg Reportado = suma de saldos; Kg Faltante = Peso KG total − Reportado (rojo); se cargan Kg Peso de Bolsas y Muestras tomadas por Laboratorio MGG; Kg No Llegó = Faltante + Bolsas + Muestras (rojo); % de lo que no llegó = No llegó / Reportado × 100 (rojo). Cada conciliación queda en un histórico con su N° y fecha, se guarda automáticamente, se puede editar y eliminar, y se sincroniza en tiempo real.',
      ...
      'Recepción Global Laboratorio: sobre la misma fila, el laboratorio carga el análisis por elemento (Sn (Estaño) — Laboratorio Mineral Group, UCV, Fe, Ti, Ta, Nb, V, Zr, Bal (estéril), Mn, Hf…). Cada elemento tiene tres lecturas A, B, C (columnas separadas) y una columna Prom. = (A + B + C) / 3; algunos (como UCV) son de valor único. Todos los valores son leyes del mineral en porcentaje (%); el Prom. y el Promedio del lote se muestran en % con mín. 2 y máx. 3 decimales. Para evitar el desplazamiento horizontal, la tabla se divide en dos mitades apiladas (mitad de los minerales arriba, el resto abajo) y el N° Análisis se repite en ambas.',
      'Añadir valores (independiente): la tabla de laboratorio es independiente de la de recepciones (kg). El botón naranja "+ Añadir valores" agrega solo una fila de análisis químico (no toca la tabla de arriba); el N° Análisis es correlativo y se incrementa solo (continúa desde el mayor valor cargado). Cada fila de análisis se puede eliminar.',
      'Configurar minerales: botón que abre un panel para gestionar los minerales/columnas del análisis: agregar uno nuevo (nombre, subtítulo opcional, tipo de columnas A/B/C/Prom. o Solo Prom., y color), editar y ocultar/mostrar. Los cambios se reflejan al instante en las columnas del laboratorio.',
      ...
      'Humedad Provisional y Humedad Final: debajo del laboratorio hay dos tablas, una al lado de la otra, con sus botones "+ Humedad Provisional" y "+ Humedad Final" para agregar filas. Provisional: se cargan Peso (Gr) Húmedos y Peso (Gr) seco; el sistema calcula % Humedad = 100 - (Peso seco / Peso Húmedos) x 4 y Merma peso H2O = Peso x % Humedad / 100; al pie, Promedio del lote del % (promedio simple) y Merma total (sumatoria). Final: se carga Peso (Kg) recogido; Merma peso H2O = Peso KG (recepciones) - Peso (Kg) recogido y el % Humedad final = Merma peso H2O / Peso KG x 100; al pie, Peso recogido y Merma totales (sumatorias). Solo la humedad se muestra en %. Cada fila se puede eliminar y se guarda en tiempo real.',
      'Añadir pesos (Bigbags): botón que abre un panel con dos tablas, Pesos Húmedos y Pesos Secos, una al lado de la otra. Con "+ Añadir BIGBAG" se agrega un bigbag con número correlativo (empieza en 1); en cada uno se indica la Procedencia (A, B, ALI, D, FALTANTE…) y su Peso húmedo y seco. Al pie de cada tabla: BIG BAG = -(cantidad de bigbags con peso) x 1.5 (resta la tara) y TOTAL NETO = suma de todos los pesos + BIG BAG (puede ser negativo). Cada bigbag se puede eliminar y se guarda en tiempo real.',

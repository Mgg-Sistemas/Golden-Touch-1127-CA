...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      'Orden de pedido: es la solicitud interna de lo que se necesita. Al crearla se indica su clasificación: Producción, Bienes o Servicios. La Unidad solicitante y el Solicitante (persona) son editables: un analista puede registrar la solicitud a nombre de otra persona o unidad; ambos se muestran en el detalle y el PDF. Al agregar un producto nuevo se elige el almacén/sub-almacén destino y la unidad desde una lista. Por cada producto se carga su finalidad y su Área (Administrativa o Producción), que aparece en el detalle de la OP (también desde la OC) y en el PDF. Se puede agregar una Nota opcional; los campos de texto se guardan en mayúscula. El botón Categorías gestiona (agregar, filtrar, editar, activar/desactivar) las clasificaciones del pedido y las unidades solicitantes; al escribir una unidad nueva se guarda sola. Si las clasificaciones superan 5, pasan a una lista buscable. Al aprobarla, el detalle y el PDF muestran quién la aprobó y cuándo.',
      ...
      'Compra directa: las compras sin proceso de aprobación pueden asociar un proveedor con buscador; si no existe, se agrega en el momento y queda guardado en el directorio de Proveedores.',
  ...
  },
  {
    ...
    puntos: [
      ...
      'Resumen caja: botón en el encabezado que abre un resumen financiero de la caja (como la hoja RESUMEN CAJA PERAMANAL GT): período (inicio, última actualización, días), saldo actual de la caja, total entregado, total gastado (gastos + nómina), tasa del material, % gastos vs % nómina, distribución de gastos y de nómina por categoría (monto y %), y bloque de Kg de casiterita (producción GT, enviados a MGG, diferencia). Se actualiza en vivo con cada movimiento y se puede descargar en PDF o enviar por correo. Incluye un filtro por rango de fechas (Desde / Hasta): al indicarlo, todo el resumen se recalcula mostrando solo los movimientos de ese rango. En "Gastos por categoría", las categorías de vehículo/maquinaria (las que terminan en "REPUESTOS - REPARACIONES - SERVICIOS", marcadas con 📊) son clicables: abren una gráfica de barras del gasto por equipo (igual que el consumo de combustible), buscable y con filtro de período.',
      ...
      'Listar movimientos (switch): la lista de Movimientos del Centro de Acopio arranca oculta. En el encabezado, junto a "+ Agregar Movimiento", hay un switch "LISTAR MOVIMIENTOS" que la muestra u oculta. Mientras está oculta, las tarjetas de resumen siguen calculándose en vivo (no hace falta activarla para verlas).',

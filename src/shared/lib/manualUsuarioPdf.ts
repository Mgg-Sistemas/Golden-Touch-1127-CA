...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Tanques — CAJA · Consumo semanal de combustible a la caja de Peramanal (automático): cada domingo el sistema suma el consumo de GT de la semana (movimientos de tanque tipo uso, valuados a su costo por tasa PMP) y registra en la caja de Peramanal abierta DOS movimientos por la misma cantidad: el gasto "CONSUMO COMBUSTIBLE GT" y una entrada de multimoneda que lo compensa. El botón "CAJA" es el respaldo manual para re-generar una semana puntual (lunes→domingo) por si no se actualizó el fin de semana y se hace el lunes; es idempotente (re-generar la misma semana reemplaza los dos movimientos, no los duplica). La compra/recepción de combustible al tanque es aparte.',

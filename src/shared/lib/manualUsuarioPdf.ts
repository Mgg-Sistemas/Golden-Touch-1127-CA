...
const SECCIONES: Seccion[] = [
  ...
  },
  {
    ...
    puntos: [
      ...
      'Compra Directa - recepción en inventario (almacenista): al pagarse una compra directa, el stock NO entra automáticamente. La compra pasa a la columna "Por recibir" del propio tablero de Compra Directa (botón "Recibir" en la tarjeta/fila) y también aparece en Inventario -> Recepciones, panel "Compras directas por recibir" (el contador del botón las incluye). Desde cualquiera de los dos, el almacenista pulsa "Recibir", ve el detalle (materiales, cantidades y costo) y elige el almacén / sub-almacén destino; al confirmar, cada material entra al inventario como ENTRADA (costo = gasto / cantidad -> PMP) en el almacén elegido y la compra pasa a Finalizada. El detalle indica si está "pendiente de recepción" o ya "recibida" (en qué almacén y por quién). Si la compra está marcada "no ingresa al inventario" (materiales ya cargados a mano), no aparece para recibir.',
      'Compra Directa en Bs - costo de inventario en USD: el inventario se valoriza en USD. Si la compra es en Bs, al darle entrada el costo de cada material se convierte a dólares con la tasa del día (Bs / tasa BCV) redondeado a 2 decimales; así el PMP y el valor del inventario quedan siempre en USD (el total a pagar en Tesorería sigue en Bs).',

          ...
          {!cargandoOrden && orden && (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '.35rem .9rem', fontSize: '.84rem' }}>
                ...
                {orden.descuento_pago_aplicado && Number(orden.descuento_pago_monto) > 0 && <div><span className="muted">Descuento:</span> <strong className="mono" style={{ color: 'var(--success)' }}>−{monto(Number(orden.descuento_pago_monto ?? 0), orden.total_moneda ?? 'USD')}</strong> <span className="muted">(incluido en el total)</span></div>}
            ...
            {!loading && rows.map((r) => {
              ...
              <tr key={r.orden.id} className="row-selectable" style={{ cursor: 'pointer' }} onClick={() => setSel(r)}>
                ...
                <td className="mono">{r.orden.oc_codigo ?? '—'}
                  ...
                  {r.orden.descuento_pago_aplicado && Number(r.orden.descuento_pago_monto) > 0 && (
                    <span className="badge" style={{ marginLeft: '.35rem', background: '#15803d', color: '#fff', fontWeight: 700, fontSize: '.62rem', padding: '.05rem .35rem' }}
                      title={`Incluye descuento: −${monto(Number(r.orden.descuento_pago_monto ?? 0), r.orden.total_moneda ?? 'USD')}`}>DESC</span>
                  )}
                </td>
  ...
  return (
    <Modal title={`✏️ Editar precios · ${o.oc_codigo ?? o.codigo}`} size="lg" onClose={() => !saving && onClose()} footer={
      ...
      <p className="muted" style={{ marginTop: 0, fontSize: '.82rem' }}>
        Ajustá el <strong>precio unitario</strong> de cada ítem y, si hace falta, <strong>agregá productos</strong> (hasta {MAX_NUEVOS}). El <strong>total a pagar</strong> se recalcula por la diferencia (se conservan IVA/IGTF/descuentos). El cambio queda en la <strong>traza</strong> de la OC y se sincroniza acá en Tesorería.
      </p>
      <div className="table-wrap">
        <table className="table" style={{ fontSize: '.82rem' }}>

...
export function CategoriasModal({ canWrite, onClose }: { canWrite: boolean; onClose: () => void }) {
  ...
  async function agregar() {
    ...
    setBusy(true);
    try { await addCatalogoPedido(tab, valor.trim().toUpperCase()); setValor(''); await recargar(); toast('Agregado', 'success'); }
    catch (e) { toast(e instanceof Error ? e.message : 'No se pudo agregar', 'error'); }
    finally { setBusy(false); }
  }
  async function guardarEdicion(id: string) {
    try { await updateCatalogoPedido(id, editValor.trim().toUpperCase()); setEditId(null); await recargar(); toast('Actualizado', 'success'); }
    ...
    <Modal title="Categorías" size="md" onClose={onClose} footer={<button className="btn btn-primary" onClick={onClose}>Cerrar</button>}>
      ...
      {canWrite && (
        <div style={{ display: 'flex', gap: '.5rem', marginBottom: '.5rem' }}>
          <input className="input" value={valor} onChange={(e) => setValor(e.target.value.toUpperCase())} placeholder={`Nueva ${tabActual.singular}…`}
      ...
      <div className="table-wrap" style={{ maxHeight: 340, overflow: 'auto' }}>
        <table className="table" style={{ fontSize: '.84rem' }}>
          ...
          <tbody>
            {!lista.length && <tr><td colSpan={canWrite ? 3 : 2} className="muted" style={{ textAlign: 'center' }}>Sin elementos.</td></tr>}
            {lista.map((l) => (
